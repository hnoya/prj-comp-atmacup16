...
class Config:
    data_path: str = "../data/"
    seed: int = 0
    experiment_name: str = "20231214_02"
    fold: int = 5
...
# %%
def get_train_arr(item_vectors: np.ndarray, yad_idxs_strs: list[str]):
    train_arrs = []
    for yad_idxs_str in yad_idxs_strs:
        yad_idxs = yad_idxs_str.split(" ")
        yad_idxs = [
            yad_id_to_item_id[int(yad_idx.strip())]
            for yad_idx in yad_idxs
            if yad_idx.strip() != ""
        ]
        # pad_yad_idx = [-1] * (10 - len(yad_idxs)) + yad_idxs
        train_arrs.append(np.mean(item_vectors[np.array(yad_idxs), :], axis=0))
    return np.array(train_arrs)
...
for fold_idx in range(Config.fold):
    ...
    yad_id_to_item_id = {
        yad_id:data.to_inner_iid(yad_id) for yad_id in (train_train["yad_no"].tolist() + test["yad_no"].tolist())

    }

    model = surprise.NMF(random_state=Config.seed, n_factors=480)
    model.fit(data)

    _train = pd.read_csv(CSVPath.log_train)
    _train["yad_no"] = _train["yad_no"].astype(str) + " "
    base_model_qi = np.mean(model.qi, axis=0)
    item_vectors = np.concatenate([model.qi, base_model_qi.reshape(1, -1)], axis=0)
    for yad_idx in pd.read_csv(CSVPath.yado)["yad_no"]:
        if yad_idx not in yad_id_to_item_id.keys():
            yad_id_to_item_id[yad_idx] = len(item_vectors) - 1

    train_arr = get_train_arr(item_vectors, _train.groupby("session_id").sum()["yad_no"].tolist())

    ratings = torch.mm(
        torch.tensor(train_arr, dtype=torch.float16).cuda(),
    ...
    pickle.dump(
        yad_id_to_item_id, open(f"../features/NMF_nfactors480_float16_{Config.experiment_name}_fold{fold_idx}_yad_id_to_item_id.pkl", "wb")
    )
...
test = pd.read_csv(CSVPath.log_test)
df = pd.concat([
        train[["session_id", "yad_no"]],
        label[["session_id", "yad_no"]],
        test[["session_id", "yad_no"]]],
).reset_index(drop=True)
df["seq_no_fixed"] = 1
df = df.rename(
...
np.save("../features/NMF_nfactors480_float16_v20231214_02.npy", ratings)
pickle.dump(
    item_id_to_yad_id, open(f"../features/NMF_nfactors480_float16_v20231214_02_item_id_to_yad_id.pkl", "wb")
)
pickle.dump(
    yad_id_to_item_id, open(f"../features/NMF_nfactors480_float16_v20231214_02_yad_id_to_item_id.pkl", "wb")
)
...
ratings = np.load("../features/NMF_nfactors480_float16_v20231214_02.npy")
import pickle

with open("../features/NMF_nfactors480_float16_v20231214_02_item_id_to_yad_id.pkl", mode="rb") as f:
    item_id_to_yad_id = pickle.load(f)
