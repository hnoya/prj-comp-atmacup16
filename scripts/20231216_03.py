...
class Config:
    data_path: str = "../data/"
    seed: int = 0
    experiment_name: str = "20231216_03"
    fold: int = 5
...
# %%
train = pd.read_csv(CSVPath.log_train)[["session_id", "yad_no"]]
label = pd.read_csv(CSVPath.label_train)[["session_id", "yad_no"]]

train_with_label = pd.concat(
    [train, label], axis=0
).reset_index(drop=True).sort_values(by="session_id", ignore_index=True)


train_with_label["label"] = 1
...
# %%
df = pd.concat([
        train_with_label[["session_id", "yad_no", "label"]],
...
model = surprise.NMF(random_state=Config.seed, n_factors=15)
model.fit(data)
